...
int iret1, iret2, id1=1, id2=2, tamVet;

int flagTroca = 0; // 0 = livre, 1 = ocupada
int turno = 1;     // 1 ou 2: quem pode entrar quando há disputa
...
void trava(int id)
{
    while (1)
    {
        while (flagTroca);  // espera se está ocupado
        flagTroca = 1;
        if (turno == id)
            break;
        flagTroca = 0;
        sched_yield();  // cede a CPU, evita busy wait total
    }
}

void destrava(int outro_id)
{
    turno = outro_id;
    flagTroca = 0;
}
...
{   
    ...
    if ((*id)==1)
    {
        ...
        {   
            // entra na seção critica para buscar e trocar
            trava(*id);
